...
class ResNet(nn.Module):
    def __init__(
        ...
        self.layers = layers
        self.num_layers = sum(self.layers) + 1
        ...
        mid_channels = 512 * block.expansion
        # segmentation head and loss function
        self.head = nn.Conv2d(mid_channels, num_classes, 1, 1)
    ...
    def forward_backbone(self, x, pool=True):
        x = self.padding(x)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
    ...
    def forward(self, inputs):
        ...
        out = self.forward_backbone(inputs, pool=False)
        out = self.head(out)
...
def resnet18(args):
    kwargs=dict(num_classes=args.nb_classes)
...
def resnet50_d16(args):
    kwargs=dict(num_classes=args.nb_classes)
    return ResNet(Bottleneck, [3, 4, 6, 3],  replace_stride_with_dilation=[False, False, True], **kwargs)
